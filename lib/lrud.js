    ...
    .filter(
      (node) =>
        !ignoredElements.some(
          (ignored) => ignored == node || ignored.contains(node)
        )
    )
...
const getAllFocusables = (scope) => [
  ...toArray(scope.querySelectorAll(focusableContainerSelector)).filter(
    (container) => getFocusables(container)?.length > 0
  ),
  ...getFocusables(scope),
];
...
const sortValidCandidates = (candidates, elem, exitDir) => {
  ...
  return candidates
    .filter((candidate) => {
      ...
      const allowedOverlap = parseFloat(
        candidate.getAttribute("data-lrud-overlap-threshold")
      );
...
const getParentFocusableContainer = (startingCandidate) => {
  ...
  do {
    ...
    startingCandidate &&
    !matches(startingCandidate, focusableContainerSelector)
  );
...
export const getNextFocus = (elem, keyOrKeyCode, scope) => {
  ...
  if (parentContainer && matches(elem, focusableSelector)) {
    ...
    getParentFocusableContainer(parentContainer)?.setAttribute(
      "data-focus",
      elem.id
    );
  }
  ...
  if (
    parentContainer?.getAttribute("data-lrud-prioritise-children") !==
      "false" &&
  ...
  if (focusableCandidates.length === 0) {
    ...
    console.log({ candidates, focusableCandidates });
  }

  for (const candidate of focusableCandidates) {
    const candidateIsContainer = matches(candidate, containerSelector);
    const candidateContainer = candidateIsContainer
      ? candidate
      : getParentContainer(candidate);
    ...
    if (!isCurrentContainer && (!isNestedContainer || candidateIsContainer)) {
      const blockedExitDirs = getBlockedExitDirs(
        parentContainer,
        candidateContainer
      );
      ...
      if (candidateContainer && !isAnscestorContainer) {
        ...
        const lastActiveChild = document.getElementById(
          candidateContainer?.getAttribute("data-focus")
        );

        const newFocus =
          lastActiveChild || getFocusables(candidateContainer)?.[0];
        getParentFocusableContainer(candidateContainer)?.setAttribute(
          "data-focus",
          newFocus?.id
        );
