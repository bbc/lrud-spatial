...
const focusableSelector = '[tabindex]:not(.lrud-ignore [tabindex]):not(.lrud-ignore), a:not(.lrud-ignore a):not(.lrud-ignore), input:not(.lrud-ignore input):not(.lrud-ignore), button:not(.lrud-ignore button):not(.lrud-ignore)';
...
export const getNextFocus = (elem, keyCode, scope) => {
  ...
  if (!bestCandidate) {
    const focusableCandidates = [
      ...
      ...toArray(scope.querySelectorAll(containerSelector)).filter(container => !!container.querySelector(focusableSelector))
    ];
