...
export const getNextFocus = (elem, keyCode, scope) => {
  ...
  const prioritisedContainer = Array.from(scope.querySelectorAll('[data-lrud-prioritise-children="false"]')).filter(container => !container.contains(elem)).length > 0;

  let focusableSiblings;
  if (prioritisedContainer) {
