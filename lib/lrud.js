...
const getValidCandidates = (elem, candidates, exitDir) => {
...
export const getNextFocus = (elem, keyCode, scope) => {
  ...
  let bestCandidate;
  const isPrioritisedContainer = !container?.hasAttribute('data-lrud-prioritise-children');

  // Get all siblings within a prioritised container
  let focusableSiblings;
  if (isPrioritisedContainer && scope.contains(container)) {
    focusableSiblings = container.querySelectorAll(focusableSelector);
  }
  ...
  if (focusableSiblings) {
    bestCandidate = getValidCandidates(elem, focusableSiblings, exitDir);
  }

  if (!bestCandidate) {
    bestCandidate = getValidCandidates(elem, focusableCandidates, exitDir);
  }
