...
export const getNextFocus = (elem, keyCode, scope) => {
  ...
  const prioritisedContainers = Array.from(scope.querySelectorAll('[data-lrud-prioritise-children="false"]'));
  let isPrioritisedContainer = true;

  // Check if any of the 'non-prioritised-containers' contain the element
  if (prioritisedContainers.length > 0) {
    prioritisedContainers.some((container) => {
      isPrioritisedContainer = container.contains(elem);
    });
  }

  // Get all siblings within a prioritised container
  let focusableSiblings;
  if (isPrioritisedContainer) {
    focusableSiblings = scope === document ? elem.parentElement.querySelectorAll(focusableSelector) : scope.querySelectorAll(focusableSelector);
  }
  ...
  const focusableCandidates = [
    ...
    ...scope.querySelectorAll(containerSelector)
  ];

  let bestCandidate = null;
  let bestDistance = Infinity;

  // Check for siblings before checking the rest of the page
  if (focusableSiblings) {
    for (let i = 0; i < focusableSiblings.length; ++i) {
      const candidate = focusableSiblings[i];
