...
export const getNextFocus = (elem, keyCode, scope) => {
  ...
  const nonPrioritisedContainer = scope.querySelectorAll('[data-lrud-non-prioritised="true"]').length > 0;

  let focusableSiblings;
  if (!nonPrioritisedContainer) {
    focusableSiblings = elem.parentElement.querySelectorAll(focusableSelector);
  }
  ...
  if (!bestCandidate) {
    for (let i = 0; i < focusableCandidates.length; ++i) {
      ...
      // Bail if the candidate is in the opposite direction or has no dimensions
      if (isValidCandidate(entryRect, entryRect, entryPointA, exitPointA)) continue;
